...
const readingSchema = z.object({
    title: z.string(),
    spreadName: z.string(),
    positionReadings: z.array(z.object({
        slotLabel: z.string(),
        cardName: z.string(),
...
function asReadingOutputCandidate(rawText, candidate) {
    const object = candidate && typeof candidate === "object" ? candidate : {};
    const rawPositionReadings = Array.isArray(object.positionReadings) ? object.positionReadings : [];
    const positionReadings = rawPositionReadings.reduce((accumulator, entry, index) => {
        if (!entry || typeof entry !== "object") {
            return accumulator;
        }
        const item = entry;
        const slotLabel = typeof item.slotLabel === "string" && item.slotLabel.trim().length > 0
            ? item.slotLabel.trim()
            : `Position ${index + 1}`;
        const cardName = typeof item.cardName === "string" && item.cardName.trim().length > 0
            ? item.cardName.trim()
            : "Unknown Card";
        const interpretation = typeof item.interpretation === "string" && item.interpretation.trim().length > 0
            ? item.interpretation.trim()
            : "";
        accumulator.push({ slotLabel, cardName, interpretation });
        return accumulator;
    }, []);
    const rawSynthesis = typeof object.synthesis === "string" && object.synthesis.trim().length > 0
        ? object.synthesis.trim()
        : rawText.replace(/```/g, "").trim();
    const looksJsonLike = rawSynthesis.startsWith("{") ||
        rawSynthesis.startsWith("[") ||
        rawSynthesis.includes('"positionReadings"') ||
        rawSynthesis.includes('"spreadName"');
    const synthesis = rawSynthesis.length > 0 && !looksJsonLike
        ? rawSynthesis
        : "The model response was truncated or malformed before valid reading JSON could be completed. Please retry the reading.";
    return {
        title: typeof object.title === "string" && object.title.trim().length > 0 ? object.title.trim() : "Reading",
        spreadName: typeof object.spreadName === "string" && object.spreadName.trim().length > 0
...
export function parseReadingResponse(rawText) {
    try {
        const json = parseJsonWithRepair(rawText);
...
export function parseTrainingResponse(rawText) {
    const json = parseJsonWithRepair(rawText);
